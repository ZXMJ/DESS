...
def processESIndex_RAG(df_squad, index):
    ...
    df_squad_sampled = df_squad.sample(n=100, random_state=20241028)  # 设置随机种子 2024 保持可重复性
    ...
    for ind in df_squad_sampled.index:

        data = df_squad_sampled.loc[ind]
...
index_name_knn = "research_index_knn_tr"
index_name = "research_index_bm25_tr"
index_name_elser = "research_index_elser_tr"

validation_dataset = pd.read_csv("/mnt/workspace/rag/RAG/date/trivia_context_sampled.csv")
df_questions = pd.DataFrame(validation_dataset)

processESIndex_RAG(df_questions, index_name_elser)
