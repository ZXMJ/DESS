...
def index_data(df_docs,source,index_name,index_name_knn,index_name_elser):
    ...
    for index, row in df_docs.iterrows():
        ...
        try:
            ...
            # 如果文档已存在，则执行更新操作
            response = es_client.update(index=index_name, id=doc_id, body={"doc": doc})
            print(f"Updated document with ID {doc_id}: {response}")
        except NotFoundError:
            # 如果文档不存在，则执行插入操作
            response = es_client.index(index=index_name, id=doc_id, body=doc)
            print(f"Indexed new document with ID {doc_id}: {response}")

        # 处理 KNN 索引
        try:
            es_client.get(index=index_name_knn, id=doc_id)
            # 如果文档已存在，则执行更新操作
            response = es_client.update(index=index_name_knn, id=doc_id, body={"doc": doc_knn})
            print(f"Updated KNN document with ID {doc_id}: {response}")
        except NotFoundError:
            # 如果文档不存在，则执行插入操作
            response = es_client.index(index=index_name_knn, id=doc_id, body=doc_knn)
            print(f"Indexed new KNN document with ID {doc_id}: {response}")
            
        # 处理 index_name_elser 索引
        try:
            es_client.get(index=index_name_elser, id=doc_id)
            # 如果文档已存在，则执行更新操作
            response = es_client.update(index=index_name_elser, id=doc_id, body={"doc": doc_elser})
            print(f"Updated elser document with ID {doc_id}: {response}")
        except NotFoundError:
            # 如果文档不存在，则执行插入操作
            response = es_client.index(index=index_name_elser, id=doc_id, body=doc_elser)
            print(f"Indexed new elser document with ID {doc_id}: {response}")
...
# 加载  squad数据集的 validation 分割
df_docs = pd.read_csv('/mnt/workspace/rag/RAG/date/squad_context_file.csv')
source ="squad"
